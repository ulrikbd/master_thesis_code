...
def plot_non_stationary_time_series():
    ...
    plt.plot(t, y, c = "k", label = "Observed series", lw = 1)
    plt.xlabel(r'$t$' + " time series example")
...
def plot_linear_trend():
    ...
    plt.plot(t, reg.predict(t), c = "b", label = "Linear trend", lw = 1)
    plt.xlabel(r'$t$')
...
def plot_cubic_splines():
    ...
    plt.plot(t, y, c = "k", alpha = 0.8,
             label = "Observed series", lw = 1)
    plt.plot(t, spl10(t), c = "r", lw = 1,
             label = "Cubic spline: " + r'$n =$' + str(n) + r'$, m =$' + str(n // m1))
    plt.plot(t, spl50(t), c = "g", lw = 1,
...
def plot_periodogram():
    ...
    plt.figure(figsize = (4.85, 2.2))
    plt.semilogy(f, Pxx_den, lw = 1)
    plt.vlines(x = [1, 3], ymin = 1e-5, ymax = 1e1, 
               colors = "red", linestyles = "dashed", alpha = 0.5, lw = 1)
    plt.xlim([0, 20])
    plt.ylim([1e-5, 1e1])
...
def plot_windows():
    ...
    plt.plot(k, w_h(k), c = "k", label = r'$W_n^H(k)$', lw = 1)
    plt.xlabel(r'$k$')
    plt.legend(loc = "upper right", handlelength=0, handletextpad=0)

    ax2 = plt.subplot(1, 2, 2)
    plt.plot(w, w_t(w), c = "k", label = r'$\mathcal{W}_n^H(\omega)$', lw = 1)
    plt.xlabel(r'$\omega$')
    plt.legend(loc = "upper right", handlelength=0, handletextpad=0)
...
def wavelet_example():
    ...
    plt.savefig("./figures/scaleogram_example.pdf", bbox_inches = "tight")
    # plt.show()
...
def watershed_example():
    ...
    plt.savefig("./figures/clustering_example.png", dpi = 1200,
                bbox_inches = "tight")
    # plt.show()
...
def main():
    seaborn.set_theme(font = "lmodern", font_scale = 0.75)
    np.random.seed(28)
